...
public class NewsDao{
           ...
           try {
                ...
                if (rs.next()) {
                    ...
                    for (String catid : catids) {
                        sql = "insert into newstype (news_id,category_id) values(?,?)";
                        smt = con.prepareStatement(sql);
    ...
    public ArrayList<News> getNewsByCatId(int catid) {
        ...
        try {
            ...
            Connection con = cp.getConnection();
            if (con != null) {
                String sql = "select * from news where id in (select news_id from newstype where category_id =?)";
                PreparedStatement smt = con.prepareStatement(sql);
                smt.setInt(1, catid);
                ResultSet rs = smt.executeQuery();
                while (rs.next()) {
     ...
     public int getNewsCountByCategory(int catid) {
        ...
        try {
            ...
            Connection con = cp.getConnection();
            if (con != null) {
                String sql = "select count(*) from newstype where category_id=?";
                PreparedStatement smt = con.prepareStatement(sql);
                smt.setInt(1, catid);
                ResultSet rs = smt.executeQuery();
                if (rs.next()) {
